    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._driver is None:
            self._driver_initializer = functools.partial(
                self._start_chrome_browser, headless=headless
            )
        else:
            for name in DriverMixin.__dict__:
                name_private = name.startswith("__") and name.endswith("__")
                name_function = isinstance(
                    DriverMixin.__dict__[name], types.FunctionType
                )
                name_in_driver = name in dir(self._driver)
                if name_private or not name_function or name_in_driver:
                    continue
                self._driver.__dict__[name] = DriverMixin.__dict__[name].__get__(
                    self._driver
                )
...
class DriverMixin:
        ...
        if not cookie_added:
            ...
            if not cookie_added:
                raise WebDriverException(
                    "Couldn't add the following cookie to the webdriver: {}".format(
                        cookie
                    )
                )
    ...
    def ensure_element(
        self, locator: str, selector: str, state: str = "present", timeout=None
    ):
