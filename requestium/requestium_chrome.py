...
class RequestiumChrome(webdriver.Chrome):
    ...
    def ensure_add_cookie(self, cookie: dict[str, str], override_domain: Optional[str] = None) -> None:
        ...
        if cookie_domain not in browser_domain:
            ...
            # TODO request. This way their server sees our ip address as continuously having the
            # TODO same cookies and not have a request mid-session with no cookies
        ...
        if not cookie_added:
            ...
            cookie_added = self.try_add_cookie(cookie)
            if not cookie_added:
                msg = f"Couldn't add the following cookie to the webdriver: {cookie!s}"
...
Use strategies from Selenium's By class (importable from selenium.webdriver.common.by).
    ...
    def xpath(self, *args, **kwargs) -> SelectorList:
        return self.selector.xpath(*args, **kwargs)

    def css(self, *args, **kwargs) -> SelectorList:
...
def _ensure_click(self) -> None:
    ...
    for _ in range(10):
        try:
            ...
            time.sleep(0.2)
    msg = f"Couldn't click item after trying 10 times, got error message: {exception_message}"
    raise WebDriverException(msg)
